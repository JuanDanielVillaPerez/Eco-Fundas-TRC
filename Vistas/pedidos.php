...
<form action=""  method="POST">
	<input type="text" name="inicio" placeholder="Inicio periodo" required>
	<input type="text" name="fin" placeholder="Fin periodo" required>
	<input type="submit" name="buscar" value="Buscar" class="btn btn-primary"><br><br>
</form>
<form action="" method="POST">
	<input type="submit" name="todos" value="Mostrar todos" class="btn btn-primary">
</form><br>
</div>
		...
		<table class="table table-striped|table-bordered|table-hover|table-sm">
			...
			<tbody style="background-color: white;">
				<?php if (isset($_POST['todos'])) {
					$query="call EcoFundas.todos_pedidos()";
					$consulta=$db->seleccionar($query);
					$resultado=$consulta->fetchAll();
					foreach ($resultado as $value) {?>
						<tr>
							<th><?php echo $value['PedidoID']; ?></th>
							<th><?php echo $value['Cliente']; ?></th>
							<th><?php echo $value['Fecha']; ?></th>
							<th><?php echo $value['Cantidad']; ?></th>
							<th><?php $costo=$value['Cantidad']*$value['Costo']; 
							echo "$ ".number_format($costo,2,".",".") ?></th>
							<th><?php echo $value['NombreMarca']; ?></th>
							<th><?php echo $value['NombreModelo']; ?></th>
							<th><?php echo $value['Tipo']; ?></th>
							<th><?php echo $value['NombreDiseño']; ?></th>
				...
				if (isset($_POST['buscar'])) {
				 	$inic_per=$_POST['inicio'];
				 	$fin_per=$_POST['fin'];
				 	$que="call EcoFundas.Reporte_pedidos_periodo('$inic_per','$fin_per')";
				 	$con=$db->seleccionar($que);
				 	$res=$con->fetchAll();
				 	foreach ($res as $fila) {?>
				 		<tr>
				 			<th><?php echo $fila['PedidoID']; ?></th>
							<th><?php echo $fila['Cliente']; ?></th>
							...
							<th><?php echo $fila['NombreDiseño']; ?></th>
